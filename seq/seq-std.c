...
int gen_need_valC()
{
    return ((icode) == (I_IRMOVL)||(icode) == (I_RMMOVL)||(icode) == (I_MRMOVL)||(icode) == (I_JXX)||(icode) == (I_CALL)||(icode) == (I_ALU));
}

int gen_instr_valid()
{
    return ((icode) == (I_NOP)||(icode) == (I_HALT)||(icode) == (I_RRMOVL)||(icode) == (I_IRMOVL)||(icode) == (I_RMMOVL)||(icode) == (I_MRMOVL)||(icode) == (I_ALU)||(icode) == (I_JXX)||(icode) == (I_CALL)||(icode) == (I_RET)||(icode) == (I_PUSHL)||(icode) == (I_POPL)||(icode) == (I_ENTER));
}
...
int gen_dstE()
{
    return (((icode) == (I_RRMOVL)||(icode) == (I_IRMOVL)||(icode) == (I_ALU)) ? (rb) : ((icode) == (I_PUSHL)||(icode) == (I_POPL)||(icode) == (I_CALL)||(icode) == (I_RET)) ? (REG_ESP) : (((icode) == (I_ENTER)) & ((ifun) == 0)) ? (REG_ESP) : (((icode) == (I_ENTER)) & ((ifun) == 1)) ? (REG_EBP) : 1 ? (REG_NONE) : 0);
}
...
int gen_aluA()
{
    return ((((icode) == (I_ALU)) & ((ra) == (REG_NONE))) ? (valc) : ((icode) == (I_ALU)) ? (vala) : ((icode) == (I_RRMOVL)) ? (vala) : ((icode) == (I_IRMOVL)||(icode) == (I_RMMOVL)||(icode) == (I_MRMOVL)) ? (valc) : ((icode) == (I_CALL)||(icode) == (I_PUSHL)) ? -4 : ((icode) == (I_RET)||(icode) == (I_POPL)) ? 4 : (((icode) == (I_ENTER)) & ((ifun) == 0)) ? -4 : (((icode) == (I_ENTER)) & ((ifun) == 1)) ? (vala) : 0);
}

int gen_aluB()
{
    return (((icode) == (I_RMMOVL)||(icode) == (I_MRMOVL)||(icode) == (I_ALU)||(icode) == (I_CALL)||(icode) == (I_PUSHL)||(icode) == (I_RET)||(icode) == (I_POPL)) ? (valb) : ((icode) == (I_RRMOVL)||(icode) == (I_IRMOVL)) ? 0 : (((icode) == (I_ENTER)) & ((ifun) == 0)) ? (valb) : (((icode) == (I_ENTER)) & ((ifun) == 1)) ? 0 : 0);
}
