...
intsig W_ifun  'mem_wb_curr->ifun'	# Instruction function
...
bool need_regids =
	f_icode in { RRMOVL, OPL, RMMOVL, MRMOVL }||f_icode == CALL && f_ifun ==2 ||f_icode == CALL && f_ifun ==3;

# Does fetched instruction require a constant word?
bool need_valC =
	f_icode in { RRMOVL, RMMOVL, MRMOVL, JXX, OPL }||f_icode == CALL && f_ifun ==0;
...
int new_E_srcA = [
	...
	D_icode == ENTER && D_ifun==1: RESP;
	D_icode in { RRMOVL, RMMOVL, OPL } : D_rA;
	D_icode == CALL && D_ifun ==2 : D_rA;
	
	D_icode == CALL && D_ifun ==3 : RESP;
	D_icode == CALL && D_ifun ==1: RESP;
...
int aluA = [
	...
	E_icode == CALL && E_ifun ==0: -4;
	E_icode == CALL && E_ifun ==2: -4;

        E_icode == CALL && E_ifun ==1: 4;
	E_icode == CALL && E_ifun ==3: 4;
...
int aluB = [
	...
	E_icode in { RMMOVL, MRMOVL, OPL, CALL } : E_valB;
	E_icode in { RRMOVL } : 0;
...
int mem_addr = [
    	...
    	M_icode in { RMMOVL, MRMOVL } : M_valE;
	M_icode == CALL && M_ifun ==0: M_valE;
	M_icode == CALL && M_ifun ==2: M_valE;

	M_icode == CALL && M_ifun ==1: M_valA;
	M_icode == CALL && M_ifun ==3: M_valA;
...
bool mem_read = M_icode in { MRMOVL }||M_icode == CALL && M_ifun ==1||M_icode == CALL && M_ifun ==3 ;

## Set write control signal
bool mem_write = M_icode in { RMMOVL }
|| (M_icode == ENTER && M_ifun == 0)
||(M_icode == CALL && M_ifun ==0)
||(M_icode == CALL && M_ifun ==2);
...
bool F_bubble =
	...
	D_icode == CALL && D_ifun == 1 ||
	E_icode == CALL && E_ifun == 1 ||
	M_icode == CALL && M_ifun == 1 ||
